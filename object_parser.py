...
def get_tomato_reviews_count(loc):
   ...
   # print(review_value)
    ...
    if len(review_values) in range (0,6):
        ...
        return(review_text+' '+review_value.rstrip())
...
def get_movie_data():
    ...
    path = '/Users/amit/Desktop/Masters/Semester 2/Web Analytics/Project/MoviePages'     
   

    listing = os.listdir(path) 

    for movies in listing:
        name = movies.split('.')
        get_name = name[0]
        data = (path+ '/' + movies)
        count = count+1
        ...
        soup = BeautifulSoup(open(data, encoding='ascii', errors='ignore').read(),'lxml')
        ...
        for loc in reviews:
            ...
            fw.write(tomato+'\t'+avg_rating+'\t'+review_count+'\t'+fresh+'\t'+rotten+'\t'+audience_score+'\t'
                     +audience_rating+'\t'+user_rating+'\t'+get_name+'\n')
        
            print('Reviews written :'+str(count))
        ...
        for con in content_body:
            ...
            content.write(get_name+'\t'+movie_rating+'\t'+genre+'\t'+director+'\t'+writer+'\t'+theatre+'\t'+stream+'\t'+box+'\t'+run+'\t'+studio+'\n') # write to file 
